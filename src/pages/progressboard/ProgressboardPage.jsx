...
function ProgressboardPage() {
  ...
  const highestScore = sortedParticipants.length > 0 ? sortedParticipants[0].score : 0;
      ...
      <Container className="py-5">
        ...
        {visibleCount < filteredParticipants.length && (
          <div className="text-center mt-4">
            <Button variant="primary" className="btn-load-more" onClick={loadMore}>
              Load Next {Math.min(ITEMS_PER_PAGE, filteredParticipants.length - visibleCount)} Participants
            </Button>
            <p className="text-muted small mt-2">
              Showing {visibleCount} of {filteredParticipants.length} participants
            </p>
          </div>
        )}
